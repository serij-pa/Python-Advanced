...
AUTORS = [
    {'autor_id': 1, 'first_name': 'Chitlur', 'last_name': 'Swaroop', 'middle_name': '_'},
    {'autor_id': 2, 'first_name': 'Herman', 'last_name': 'Melville', 'middle_name': '_'},
    {'autor_id': 3, 'first_name': 'Leo', 'last_name': 'Tolstoy', 'middle_name': 'Nikolaevich'}
]

DATABASE_NAME = 'table_books.db'
BOOKS_TABLE_NAME = 'books'
AUTORS_TABLE_NAME = 'autors'
...
def init_db(initial_autors, initial_books):
    with sqlite3.connect(DATABASE_NAME) as conn:
        ...
        cursor.execute(
            ...
            WHERE type='table' AND name='{AUTORS_TABLE_NAME}';
            """
        )
        ...
        if not exists:
            cursor.executescript(
                f"""
                CREATE TABLE `{AUTORS_TABLE_NAME}`(
            ...
            cursor.executemany(
                f"""
                INSERT INTO `{AUTORS_TABLE_NAME}`
                (first_name, last_name, middle_name) VALUES (?, ?, ?)
                """,
                [
                    (item['first_name'], item['last_name'], item['middle_name'])
                    for item in initial_autors
                ]
            )
...
def _get_autor_obj_from_row(row: tuple):
    return Autor(autor_id=row[0], first_name=row[1], last_name=row[2], middle_name=row[3])
...
def get_all_autors():
    with sqlite3.connect(DATABASE_NAME) as conn:
        cursor = conn.cursor()
        cursor.execute(f'SELECT * FROM `{AUTORS_TABLE_NAME}`')
        all_autors = cursor.fetchall()
        return [_get_autor_obj_from_row(row) for row in all_autors]
...
def add_autor(autor: Autor):
    with sqlite3.connect(DATABASE_NAME) as conn:
        ...
        cursor.execute(
            ...
            (first_name, last_name, middle_name) VALUES (?, ?)
            """,
            (autor.first_name, autor.last_name, autor.middle_name)
        )
        autor.autor_id = cursor.lastrowid
        return autor
