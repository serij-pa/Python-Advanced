...
def create_table_description_book(initial_description):
    with sqlite3.connect('table_books.db') as conn:
        ...
        cursor.execute(
            """
            SELECT name FROM sqlite_master
            WHERE type='table' AND name='descript'; 
            """)
        ...
        if not exists:
            ...
            """)
            cursor.executemany("INSERT INTO descript (description, view_counter, book_number) VALUES (?, ?, ?)",
...
def get_all_descriptions():
    with sqlite3.connect('table_books.db') as conn:
        cursor = conn.cursor()
        descriptions = cursor.execute("SELECT * FROM descript").fetchall()
        return descriptions
...
def description(book_id):
    with sqlite3.connect('table_books.db') as conn:
        cursor = conn.cursor()
        result = cursor.execute(f"SELECT * FROM descript WHERE book_number = {book_id}").fetchone()
        if result is not None:
            cursor.execute(f"UPDATE descript SET view_counter = {result[2] + 1} WHERE book_number = {book_id}")
        return result
