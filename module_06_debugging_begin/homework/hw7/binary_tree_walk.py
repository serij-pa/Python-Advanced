...
def restore_tree(path_to_log_file: str) -> BinaryTreeNode:
    with open(path_to_log_file, "r") as file:
        ...
        #number_root = int(lines[0][30:36])
        number_root = list(map(int, re.findall(r"\d+", lines[0])))[0]
        root = BinaryTreeNode(val=number_root, left=None, right=None)
        tree = {}
        for lin in lines:
            if "INFO" in lin:
                #number_node = int(lin[30:36])
                number_node = list(map(int, re.findall(r"\d+", lin)))[0]
                tree[number_node] = BinaryTreeNode(number_node)

            elif "DEBUG" in lin and "left" in lin:
                #left_node = int(lin[73:79])
                left_node = list(map(int, re.findall(r"\d+", lin)))[1]
                tree[number_node] = BinaryTreeNode(val=number_node,
                                                   left=BinaryTreeNode(left_node),
                                                   right=None)

            elif "DEBUG" in lin and "right" in lin:
                #right_node = int(lin[74:80])
                right_node = list(map(int, re.findall(r"\d+", lin)))[1]
                tree[number_node] = BinaryTreeNode(val=number_node,
                                                   left=BinaryTreeNode(left_node),
                                                   right=BinaryTreeNode(right_node))
        return root
